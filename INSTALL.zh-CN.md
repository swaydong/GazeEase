...
本文说明当前的私有本机分发流程。安装包使用供开发者本人 Mac 使用的 Apple Development 签名，没有使用 Developer ID，也没有经过 Apple 公证，不适合公开分发。
...
7. 安装成功后，GazeEase 会从 `/Applications/GazeEase.app` 打开。
...
GazeEase 需要“输入监控”来判断活动和休息中断。该权限完全由 macOS 控制，安装器不能代替用户授予。

1. 打开 GazeEase。
2. 按 App 引导操作，或进入“系统设置 → 隐私与安全性 → 输入监控”。
3. 开启 GazeEase。
4. 如果状态没有立即刷新，退出并重新打开 GazeEase。

GazeEase 不需要“屏幕录制”权限。
...
1. 先打开 GazeEase 设置，关闭“登录时启动”，让 App 正确向 macOS 注销登录项。
...
只有在已经退出并卸载 GazeEase、且确认不希望以后重装时保留历史的情况下，才执行以下步骤。卸载脚本刻意不会自动完成这些操作。
